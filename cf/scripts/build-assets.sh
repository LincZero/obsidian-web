...
# ── Generate docs-generated.js from cf/docs/ ─────────────────────────────
# Reads markdown/text files from cf/docs/ and generates a JS module exporting
# them as a Map, similar to how plugins are handled.
echo "  generating docs bundle..."
DOCS_DIR="$CF_DIR/docs"
DOCS_OUT="$CF_DIR/src/docs-generated.js"

cat > "$DOCS_OUT" <<'HEADER'
// AUTO-GENERATED by build-assets.sh — do not edit manually.
// Contains doc files as strings, keyed by relative path.
export const DOCS_FILES = new Map([
HEADER
...
DOCS_COUNT=$(grep -c '^\[' "$DOCS_OUT" 2>/dev/null || echo 0)
DOCS_SIZE=$(du -sh "$DOCS_OUT" 2>/dev/null | cut -f1)
echo "  docs: $DOCS_COUNT files ($DOCS_SIZE)"
